...
export default function HoldingsView({ holdings }: { holdings: Holding[] }) {
  ...
  if (!holdings?.length) {
    return (
      <div className="flex flex-col items-center justify-center h-40 text-white/20 font-mono text-sm">
        ...
        return (
          <div
            ...
            className={`relative flex items-center gap-4 p-4 rounded-xl border transition-all cursor-default overflow-hidden ${
              isHov
                ? 'border-[#0ff2]/30 bg-[#0ff2]/5'
                : 'border-[#0ff2]/08 bg-[#050d12]/60'
            }`}
          >
            {/* Weight bar */}
            <div
            ...
            {/* Symbol badge */}
            <div
              className={`relative z-10 w-12 h-12 rounded-lg flex items-center justify-center font-mono font-black text-xs tracking-widest flex-shrink-0 ${
                isPositive
            ...
            <div className="relative z-10 flex-1 min-w-0">
              <div className="flex items-baseline gap-2">
                <span className="text-white font-mono font-bold text-sm">{h.symbol}</span>
                <span className="text-white/30 text-xs font-mono">{h.shares.toFixed(4)} shares</span>
              </div>
            ...
            <div className="relative z-10 text-right flex-shrink-0">
              <div className="text-white font-mono font-bold text-sm">${h.market_value.toFixed(2)}</div>
            ...
            <div className="relative z-10 flex-shrink-0">
              <div className="text-white/20 text-xs font-mono">{h.weight.toFixed(1)}%</div>
            </div>
          </div>
        );
      })}
